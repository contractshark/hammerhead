...
library ConfigLibrary {
    ...
    function remove(
        ...
        uint256 removeIndex = self.entries[key].index;
...
contract OptinoBase is Owned {
    ...
    function configDataLength() public view returns (uint256) {
        return configData.length();
    }

    function getConfigByIndex(uint256 i)
        public
        view
        returns (
            bytes32,
    ...
    // }
    function seriesDataLength() public view returns (uint256) {
    ...
    function _getSeries(TradeData memory tradeData) internal view returns (SeriesLibrary.Series storage) {
        bytes32 key = SeriesLibrary.generateKey(
            tradeData.baseToken,
            tradeData.quoteToken,
            tradeData.priceFeed,
            tradeData.callPut,
...
contract VanillaOptino is Orders {
    ...
    function tradeDataLength() public view returns (uint256) {
        return trades.length;
    }

    function getTrade(uint256 i)
        public
        view
        returns (
            address,

...
contract(
  'TestableIdleDAI (internal functions exposed as public)',
  function ([_, registryFunder, creator, nonOwner, someone]) {
    ...
    it('_mintCTokens', async function () {
      ...
      assert.equal(
        BNify(DAIBalance).toString(),
        expectedBalance.toString(),
        'DAI balance is correct for owner',
      );
    ...
    it('_redeemCTokens', async function () {
      ...
      await this.DAIMock.transfer(this.cDAIMock.address, BNify('100').mul(this.one), {
        from: creator,
      });
      ...
      const res = await this.token._redeemCTokens.call(cDAIBalance, this.token.address, {
        from: creator,
      });
    ...
    it('_redeemITokens', async function () {
      ...
      await this.DAIMock.transfer(this.token.address, this.one, { from: creator });
      // owner transfers 100 DAI to the contract
      await this.DAIMock.transfer(this.iDAIMock.address, BNify('100').mul(this.one), {
        from: creator,
      });

      const res = await this.token._redeemITokens.call(this.one, this.token.address, {
        from: creator,
      });

...
contract('JS Phoenix test', async (accounts) => {
  ...
  it('Constructor and Destructor', async function () {
    ...
    let typical_use_case_phoenix = await Phoenix.new(owner, spender1, AVE_DELAY, 10, {
      from: spender1,
    });
    ...
    await truffleAssert.reverts(
      Phoenix.new(owner, spender1, MIN_DELAY / 2, 10, { from: spender1 }),
    ); // delay too small
    await truffleAssert.reverts(
      Phoenix.new(owner, spender1, MIN_DELAY - 1, 10, { from: spender1 }),
    ); // delay too small
    await truffleAssert.reverts(
      Phoenix.new(owner, spender1, MAX_DELAY + 1, 10, { from: spender1 }),
    ); // delay too big
    await truffleAssert.reverts(
      Phoenix.new(owner, spender1, 100 * MAX_DELAY, 10, { from: spender1 }),
    ); // delay too big
  ...
  it('addTransaction()', async function () {
    ...
    await truffleAssert.reverts(
      phoenix.addTransaction(owner, web3.utils.toWei('1', 'ether'), { from: hacker }),
    ); // only tier 2 can do it
    await truffleAssert.reverts(
      phoenix.addTransaction(owner, web3.utils.toWei('1', 'ether'), { from: owner }),
    ); // only tier 2 can do it
    await truffleAssert.reverts(
      phoenix.addTransaction(owner, web3.utils.toWei('4', 'ether'), { from: spender1 }),
    ); // not enough money
    await truffleAssert.reverts(
      phoenix.addTransaction(phoenix.address, web3.utils.toWei('4', 'ether'), { from: spender1 }),
    ); // phoenix cannot pay itself
    await truffleAssert.reverts(
      phoenix.addTransaction(
    ...
    await truffleAssert.passes(phoenix.lock(2));
    await truffleAssert.reverts(
      phoenix.addTransaction(owner, web3.utils.toWei('2', 'ether'), { from: spender1 }),
    ); // locked
    test_util.mineNBlocks(2);

    let call_res = await phoenix.addTransaction(spender2, web3.utils.toWei('1', 'ether'), {
      from: spender1,
    });
    truffleAssert.eventEmitted(call_res, 'Request', (ev) => {
      return (
        ev.to == spender2 &&
        ev.amount == web3.utils.toWei('1', 'ether') &&
        ev.id == 1 &&
        ev.from == spender1
      );
    });

    call_res = await phoenix.addTransaction(spender1, web3.utils.toWei('1', 'ether'), {
      from: spender1,
    });
    truffleAssert.eventEmitted(call_res, 'Request', (ev) => {
      return (
    ...
    await truffleAssert.reverts(
      phoenix.addTransaction(hacker, web3.utils.toWei('1', 'ether'), { from: spender1 }),
    ...
    await truffleAssert.reverts(
      phoenix.addTransaction(hacker, web3.utils.toWei('1', 'ether'), { from: spender1 }),
    ); //transaction list is still full, no payment was comitted
  });
